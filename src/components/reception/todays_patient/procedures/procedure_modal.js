...
import { AppointmentCharges, AppointmentCheckout, AppointmentSearchById, AppointmentUpdateCharges, ProcedureCreate, ProcedureSearchByAppointmentId, ProcedureUpdate } from '../../../../shared/queries';
...
class ProcedureModal extends Component {
    ...
    handle_close_modal = () => {
        for (let i = 0; i < this.state.procedures_list.length; ++i) {
            if (this.state.procedures_list[i]._id === null) {
                this.props.notify('info', '', 'There are unsaved procedures. Either delete them or save them.')
                return;
            }
        }
        this.setState({
    ...
    on_save_changes_click = async () => {
        ...
        for (let i = 0; i < this.state.procedures_list.length; ++i) {
            if ((this.state.procedures_list[i].discount) > this.state.procedures_list[i].fee) {
                this.props.notify('error', '', `Discount cannot be more than the charges in procedure number ${i + 1}`);
                error = true;
            }
            else if (this.state.procedures_list[i].dr_share > 100) {
                this.props.notify('error', '', 'Doctor share cannot be more than 100%');
                error = true;
            }
        }
        if (error) {
    ...
    on_save_payment_click = () => {
        ...
        if (parseInt(this.state.paid_text_input.value) < this.state.total) {
            this.props.notify('info', '', 'Paid amount is insufficient');
            return;
        }
